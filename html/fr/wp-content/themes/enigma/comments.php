...
<?php if ( comments_open() ) : ?>
	...
	add_filter('wl_comment_form_default_fields','my_fields');
		...
		'logged_in_as' => '<p class="logged-in-as">' . __( "Connecté en tant que ",'weblizar' ).'<a href="'. admin_url( 'profile.php' ).'">'.$user_identity.'</a>'. '<a href="'. wp_logout_url( get_permalink() ).'" title="Se déconnecter">'.__(", se déconnecter?",'weblizar').'</a>' . '</p>',
		'title_reply_to' => __( 'Leave a Reply to %s','weblizar'),
		'id_submit' => 'enigma_send_button',
		'label_submit'=>__( 'Publier','weblizar'),
		'comment_notes_before'=> '',
		'comment_notes_after'=>'',
		'title_reply'=> '<h2>'.__('Déposer un commentaire','weblizar').'</h2>',		
		'role_form'=> 'form',		
		);
